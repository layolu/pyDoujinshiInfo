...
class API:
    ...
    def __init__(self, refresh_deadline_sec=30,
        ...
        self._api = tortilla.wrap(self.ENDPOINT, cache=tortilla_cache, cache_lifetime=tortilla_cache_lifetime,
    ...
    def do_refresh_token(self) -> None:
        ...
        if not self.expires_at or (self.expires_at - datetime.now()) < timedelta(seconds=self.refresh_deadline_sec):
            res: Bunch = self._api.auth.login.post(params={
    ...
    def get_tag(self, tag_type: str, slug: str, page=1, limit=24) -> PaginatedResults:
        return PaginatedResults(self._api.tag(tag_type)(slug).get, paginated_key='books',
                                params={'page': page, 'limit': limit})

    def get_tag_changelog(self, tag_type: str, slug: str, page=1, limit=24) -> PaginatedResults:
        return PaginatedResults(self._api.tag(tag_type)(slug).changelog.get, params={'page': page, 'limit': limit})
    ...
    def update_tag(self, tag_type: str, slug: str, name_japanese: str, tags: List[str] = [],
        ...
        for key in ('name_romaji', 'name_english', 'aliases', 'description_english',
            ...
            if key in kwargs:
                data[key] = kwargs[key]
        self.do_refresh_token()
        return PaginatedResults(self._api.tag(tag_type)(slug).put, paginated_key='books', data=data)
    ...
    def create_doujinshi(self, name_japanese: str, tag_ids: List[str] = [], cover: Optional[BinaryIO] = None,
    ...
    def get_changelog(self, contribution_id: str) -> Bunch:
        return self._api.changelog(contribution_id).get()
    ...
    def search_tags(self, tag_type: str, query: str, page=1, limit=24) -> PaginatedResults:
        params = {'q': query, 'page': page, 'limit': limit}
        return PaginatedResults(self._api.search.tag(tag_type).get, params=params)
    ...
    # User Libraries
    def get_users_library(self, slug: str, lib_type: str, page=1, limit=24) -> PaginatedResults:
        return PaginatedResults(self._api.user(slug).library(lib_type).get, params={'page': page, 'limit': limit})

    def check_library_entry(self, lib_type: str, book_id: str) -> bool:
        self.do_refresh_token()
        return self._api.library(lib_type)(book_id).get().data

    def add_library_entry(self, lib_type: str, book_id: str) -> None:
        self.do_refresh_token()
        self._api.library(lib_type).post(data={'book': book_id})

    def remove_library_entry(self, lib_type: str, book_id: str) -> None:
        self.do_refresh_token()
        self._api.library(lib_type).delete(data={'book': book_id})
